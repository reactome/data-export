...
public class ProteinRoleReaction extends DataExportAbstract {
    ...
    public String getQuery() {
        return "MATCH (rd:ReferenceDatabase)<--(n)<-[:referenceEntity|referenceSequence|crossReference|referenceGene*]-(pe:PhysicalEntity) "+
                ...
                "UNWIND pes AS pe "+
                "MATCH (pe)<-[:physicalEntity|regulator|hasComponent|hasMember|hasCandidate|repeatedUnit*]-()<-[r:input|output|catalystActivity|regulatedBy|entityFunctionalStatus]-(rle:ReactionLikeEvent) "+
